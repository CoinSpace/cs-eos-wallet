...
import {
  validateAccountName,
  validateDestinationAccountName,
  validateMemo,
} from './validator.js';
...
class EOSTransaction extends Transaction {
  get url() {
    if (this.development) {
      return `https://testnet.explorer.xprnetwork.org/transaction/${this.id}`;
    }
    return `https://eosauthority.com/transaction/${this.id}?network=eos`;
  }
}
...
export default class EOSWallet extends CsWallet {
  ...
  async validateAddress({ address }) {
    ...
    if (!validateDestinationAccountName(address)) {
  ...
  async createTransaction({ address, meta = {}, amount }, seed) {
    ...
    try {
      ...
      if (typeof err?.response === 'string') {
        if (/^Account does not exist/.test(err.response)) {
          throw new errors.DestinationAccountError(err.response);
        } else if (/^Expired transaction/.test(err.response)) {
          throw new errors.ExpiredTransactionError(err.response);
        } else if (/^CPU usage exceeded/.test(err.response)) {
          throw new errors.CPUExceededError(err.response);
        } else if (/^NET usage exceeded/.test(err.response)) {
          throw new errors.NETExceededError(err.response);
        }
      }
      throw err;
    }
