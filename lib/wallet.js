...
function Wallet(options) {
  ...
  this.txsPerPage = options.txsPerPage || 50;
  this.api = new API();
  this.balance = '0';
  this.fee = '0';
  this.isActive = false;
  ...
  that.api.accounts.validate(options.accountName || '_', that.account.owner.publicKey).then(function(isValid) {
    if (!isValid) {
      done(null, that);
      txDone(null, that);
      return;
    ...
    return that.loadTxs(that.account.name).then(function(data) {
...
Wallet.prototype.loadTxs = function() {
  var that = this;
  return this.api.accounts.txs(that.account.name, that.txsCursor, that.txsPerPage).then(function(data) {
...
Wallet.deserialize = function(json) {
  ...
  wallet.hasMoreTxs = deserialized.hasMoreTxs;
