...
Wallet.prototype.load = function(options) {
  ...
  that.api.accounts.validate(that.accountName, that.account.owner.publicKey).then(function(isValid) {
    if (!isValid) {
      done(null, that);
      return;
    }
    that.account.name = that.accountName;
...
Wallet.prototype.loadTxs = function() {
  ...
  return this.api.accounts.txs(that.account.name, that.txsCursor).then(function(data) {
    data.txs = data.txs.map(function(tx) {
      ...
      tx.isIncoming = tx.to === that.account.name && tx.from !== tx.to;
      return tx;
    });
...
Wallet.prototype.sendTx = function(tx, done) {
  var that = this;
  that.api.transactions.serialize(tx.data).then(function(serializedTransaction) {
    var requiredKeys = tx.signatureProvider.availableKeys;
    ...
    return that.api.transactions.propagate(pushTransactionArgs).then(function() {
      that.balance = Big(that.balance).minus(tx.data[0].data.quantity.split(' ')[0]).toFixed();
      done(null);
    }).catch(function(err) {
