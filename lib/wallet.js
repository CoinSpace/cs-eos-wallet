...
class Wallet {
  ...
  load(options) {
    ...
    this.api.accounts.validate(this.accountName, this.account.owner.publicKey).then((isValid) => {
      if (!isValid) {
        done(null, this);
        return;
      }
      this.account.name = this.accountName;
      return this.api.accounts.info(this.account.name).then((data) => {
  ...
  loadTxs() {
    ...
    return this.api.accounts.txs(this.account.name, this.txsCursor).then((data) => {
  ...
  sendTx(tx, done) {
    this.api.transactions.serialize(tx.data).then((serializedTransaction) => {
      const requiredKeys = tx.signatureProvider.availableKeys;
      return tx.signatureProvider.sign({
        chainId: this.chainId,
        requiredKeys,
        serializedTransaction: Buffer.from(serializedTransaction, 'hex'),
      });
    }).then((pushTransactionArgs) => {
      return this.api.transactions.propagate(pushTransactionArgs).then(() => {
        this.balance = Big(this.balance).minus(tx.data[0].data.quantity.split(' ')[0]).toFixed();
        done(null);
      }).catch((err) => {
